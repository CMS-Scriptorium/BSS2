...
require_once(WB_PATH.'/modules/bakery/resize_img.php');
require_once(WB_PATH.'/modules/bakery/pngthumb.php');
...
$item_order = new order('{BXT}_items', 'position', 'item_id', 'section_id');



// Remove any tags and add slashes
$old_link          = lazystrip($admin->get_post('link'));
$old_section_id    = lazystrip($admin->get_post('section_id'));
$new_section_id    = lazystrip($admin->get_post('new_section_id'));
$action            = lazystrip($admin->get_post('action'));

$title             = $database->escapeString(lazystrip($admin->get_post('title')));
$sku               = $database->escapeString(lazystrip($admin->get_post('sku')));
$stock             = $database->escapeString(lazystrip($admin->get_post('stock')));
$price             = $database->escapeString(lazystrip($admin->get_post('price')));
$shipping          = $database->escapeString(lazystrip($admin->get_post('shipping')));
$tax_rate          = $database->escapeString(lazystrip($admin->get_post('tax_rate')));
$active            = lazystrip($admin->get_post('active'));
$definable_field_0 = $database->escapeString(lazystrip($admin->get_post('definable_field_0')));
$definable_field_1 = $database->escapeString(lazystrip($admin->get_post('definable_field_1')));
$definable_field_2 = $database->escapeString(lazystrip($admin->get_post('definable_field_2')));
$description       = $database->escapeString(lazystrip($admin->get_post('description')));
$full_desc         = $database->escapeString($admin->get_post('full_desc'));
...
if (!empty($_POST['images'])) {
	foreach ($_POST['images'] as $img_id => $image) {
		...
		$image = array_map(array($database, 'escapeString'), $image);
		// Sanitize vars
...
$imgresize    = lazystrip($admin->get_post('imgresize'));
$quality      = lazystrip($admin->get_post('quality'));
$maxheight    = lazystrip($admin->get_post('maxheight'));
$maxwidth     = lazystrip($admin->get_post('maxwidth'));

$attribute_id = $database->escapeString(lazystrip($admin->get_post('attribute_id')));
$ia_operator  = $database->escapeString(lazystrip($admin->get_post('ia_operator')));
$ia_price     = $database->escapeString(lazystrip($admin->get_post('ia_price')));
$ia_price     = empty($ia_price) ? 0 : $ia_price;
$assign_id    = $database->escapeString(lazystrip($admin->get_post('assign_id')));
...
if (empty($admin->get_post('title'))) {
	...
	$_SESSION['bxt']['item']['title']             = $title;
	$_SESSION['bxt']['item']['sku']               = $sku;
	$_SESSION['bxt']['item']['stock']             = $stock;
	$_SESSION['bxt']['item']['price']             = $price;
	$_SESSION['bxt']['item']['shipping']          = $shipping;
	$_SESSION['bxt']['item']['tax_rate']          = $tax_rate;
	$_SESSION['bxt']['item']['active']            = $active;
	$_SESSION['bxt']['item']['definable_field_0'] = $definable_field_0;
	$_SESSION['bxt']['item']['definable_field_1'] = $definable_field_1;
	$_SESSION['bxt']['item']['definable_field_2'] = $definable_field_2;
	$_SESSION['bxt']['item']['description']       = $description;
	$_SESSION['bxt']['item']['full_desc']         = $full_desc;
	$_SESSION['bxt']['item']['images']            = $images;
	$_SESSION['bxt']['item']['imgresize']         = $imgresize;
	$_SESSION['bxt']['item']['quality']           = $quality;
	$_SESSION['bxt']['item']['maxheight']         = $maxheight;
	$_SESSION['bxt']['item']['maxwidth']          = $maxwidth;
	$_SESSION['bxt']['item']['new_section_id']    = $new_section_id;
	$_SESSION['bxt']['item']['action']            = $action;
...
if ($old_section_id != $new_section_id && $action == 'move') {
	...
	$query_sections = $database->query("SELECT `page_id` FROM `{TP}sections` WHERE `section_id` = '$new_section_id'");
...
$module_pages_directory = $database->get_one("SELECT `pages_directory` FROM `{BXT}_general_settings`");
...
if (!empty($_POST['save_attribute'])) {
	...
	$query_attributes = $database->query("SELECT `option_id` FROM `{BXT}_attributes` WHERE `attribute_id` = '$attribute_id'");
	$attribute = $query_attributes->fetchRow();
	$option_id = lazystrip($attribute['option_id']);

	// Update item attribute
	if (isset($_POST['attribute_id'])) {
		if (isset($_POST['assign_id']) && is_numeric($_POST['assign_id'])) {
			$database->query("UPDATE `{BXT}_item_attributes` SET `option_id` = '$option_id', `attribute_id` = '$attribute_id', `price` = '$ia_price', `operator` = '$ia_operator' WHERE `item_id` = '$item_id' AND `assign_id` = '$assign_id'");
		}
		// Insert new item attribute
		else {
			$database->query("INSERT INTO `{BXT}_item_attributes` (`item_id`, `option_id`, `attribute_id`, `price`, `operator`) VALUES ('$item_id', '$option_id', '$attribute_id', '$ia_price', '$ia_operator')");
		}
	}
...
if (!is_writable(WB_PATH.PAGES_DIRECTORY.$module_pages_directory)) {
	...
	$pages_dir_depth = count(lazyexplode('/',PAGES_DIRECTORY))-1;
...
require_once("'.$index_location.'config.php");
...
foreach ($images as $img_id  => $image) {
	if ($image['delete_image'] !== false) {
		...
		$database->query("DELETE FROM {BXT}_images WHERE img_id = '$img_id'");
		// Check if there was a db error
		if ($database->is_error()) {
			$errors[] = $database->get_error();
		}
...
$resize = $database->get_one("SELECT `resize` FROM `{BXT}_page_settings` WHERE `section_id` = '$section_id'");
...
for ($i = 0; $i < $num_images; $i++) {
	if (isset($_FILES['image']['tmp_name'][$i]) AND !empty($_FILES['image']['tmp_name'][$i])) {
		...
		if (!preg_match('#^[a-zA-Z0-9._-]*$#', $filename)) {
			$errors[] = $TXT_BAKERY['ERR_INVALID_FILE_NAME'].': '.lazyspecial($filename.'.'.$fileext);
			continue;
		}
		// Check lenght of filename
		if (strlen($filename) > $filename_max_length) {
			$errors[] = $TXT_BAKERY['ERR_FILE_NAME_TOO_LONG'].': '.lazyspecial($filename.'.'.$fileext);
			continue;
		}
		...
		$top_position = $database->get_one("SELECT MAX(`position`) AS `top_position` FROM `{BXT}_images` WHERE `item_id` = '$item_id'");
		// Increment position (db function returns NULL if this item has no image yet)
		$top_position = intval($top_position) + 1;

		// Insert file into database
		$filename = $filename.'.'.$fileext;
		$database->query("INSERT INTO `{BXT}_images` (`item_id`, `filename`, `position`) VALUES ('$item_id', '$filename', '$top_position')");
		// Check if there was a db error
		if ($database->is_error()) {
			$errors[] = $database->get_error();
		}
...
if ($action == 'duplicate') {
	...
	$database->query("INSERT INTO `{BXT}_items` (`section_id`, `page_id`, `position`, `created_when`, `created_by`) VALUES ('$section_id', '$page_id', '$position','".time()."','".$admin->get_user_id()."')");
	...
	$query_item_attributes = $database->query("SELECT * FROM `{BXT}_item_attributes` WHERE `item_id` = '$orig_item_id'");
	if ($query_item_attributes->numRows() > 0) {
		while ($ia = $query_item_attributes->fetchRow()) {  // ia = item_attributes
			...
			$database->query("INSERT INTO `{BXT}_item_attributes` (`item_id`, `option_id`, `attribute_id`, `price`, `operator`) VALUES ('$item_id', '{$ia['option_id']}', '{$ia['attribute_id']}', '{$ia['price']}', '{$ia['operator']}')");
		}
	}
...
?>';
	...
	$query_images = $database->query("SELECT * FROM `{BXT}_images` WHERE `item_id` = '$orig_item_id'");
	if ($query_images->numRows() > 0) {
		while ($image = $query_images->fetchRow()) {
			// Insert duplicated images
			$database->query("INSERT INTO `{BXT}_images` (`item_id`, `item_attribute_id`, `filename`, `active`, `position`, `alt`, `title`, `caption`) VALUES ('$item_id', '{$image['item_attribute_id']}', '{$image['filename']}', '{$image['active']}', '{$image['position']}', '{$image['alt']}', '{$image['title']}', '{$image['caption']}')");
		}
	}
