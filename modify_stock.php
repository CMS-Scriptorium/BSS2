...
<form name="category" action="<?=WB_URL; ?>/modules/bakery/modify_stock.php" method="post" style="margin: 0;">
<input type="hidden" name="page_id" value="<?=$page_id; ?>" />
<h2 style="display: inline;"><?=$TXT_BAKERY['STOCK_ADMIN']; ?></h2>
<select name="cat" onchange="javascript: document.category.submit();" style="width: 360px; margin: 0 0 15px 100px;">
<option value=""><?=$TXT_BAKERY['ALL']. ' ' . $TXT_BAKERY['ITEMS']; ?></option>
<?=$sections_select; ?>
</select>
</form>
...
$order = "ASC";
$text_order = $TXT_BAKERY['ORDER_DESC'];
if (isset($_GET['order'])) {
	$order = $_GET['order'] == "ASC" ? "DESC" : "ASC";
	$text_order = $_GET['order'] == "ASC" ? $TXT_BAKERY['ORDER_ASC'] : $TXT_BAKERY['ORDER_DESC'];
}
...
if ($query_items->numRows() > 0) {
	...
	<tr height="30" valign="bottom" class="mod_bakery_submit_row_b">
      <th align="left"><a href="<?=WB_URL; ?>/modules/bakery/modify_stock.php?page_id=<?=$page_id; ?>&amp;section_id=<?=$section_id; ?>&amp;order_by=sku&amp;order=<?=$order; ?>&amp;cat=<?=$category; ?>" title="<?=$text_order; ?>"><?=$TXT_BAKERY['SKU']; ?></a></th>
      <th align="center"><a href="<?=WB_URL; ?>/modules/bakery/modify_stock.php?page_id=<?=$page_id; ?>&amp;section_id=<?=$section_id; ?>&amp;order_by=modified_when&amp;order=<?=$order; ?>&amp;cat=<?=$category; ?>" title="<?=$text_order; ?>"><?=$TEXT['DATE']; ?></a></th>
      <th align="left"><a href="<?=WB_URL; ?>/modules/bakery/modify_stock.php?page_id=<?=$page_id; ?>&amp;section_id=<?=$section_id; ?>&amp;order_by=title&amp;order=<?=$order; ?>&amp;cat=<?=$category; ?>" title="<?=$text_order; ?>"><?=$TXT_BAKERY['NAME']; ?></a></th>
      <th align="left"><a href="<?=WB_URL; ?>/modules/bakery/modify_stock.php?page_id=<?=$page_id; ?>&amp;section_id=<?=$section_id; ?>&amp;order_by=price&amp;order=<?=$order; ?>&amp;cat=<?=$category; ?>" title="<?=$text_order; ?>"><?=$TXT_BAKERY['PRICE']; ?></a></th>
      <th align="left"><a href="<?=WB_URL; ?>/modules/bakery/modify_stock.php?page_id=<?=$page_id; ?>&amp;section_id=<?=$section_id; ?>&amp;order_by=shipping&amp;order=<?=$order; ?>&amp;cat=<?=$category; ?>" title="<?=$text_order; ?>"><?=$TXT_BAKERY['SHIPPING']; ?></a></th>
      <th align="center"><a href="<?=WB_URL; ?>/modules/bakery/modify_stock.php?page_id=<?=$page_id; ?>&amp;section_id=<?=$section_id; ?>&amp;order_by=stock&amp;order=<?=$order; ?>&amp;cat=<?=$category; ?>" title="<?=$text_order; ?>"><?=$TXT_BAKERY['IN_STOCK']; ?></a></th>
      <th align="left"><a href="<?=WB_URL; ?>/modules/bakery/modify_stock.php?page_id=<?=$page_id; ?>&amp;section_id=<?=$section_id; ?>&amp;order_by=active&amp;order=<?=$order; ?>&amp;cat=<?=$category; ?>" title="<?=$text_order; ?>"><?=$TEXT['ACTIVE']; ?></a></th>
...
<table width="98%" align="center" cellpadding="0" cellspacing="0" class="mod_bakery_submit_row_b">
	<tr valign="top">
	  ...
	  <input type="button" value="<?=$TEXT['RELOAD']; ?>" onclick="javascript: window.location = '<?=WB_URL; ?>/modules/bakery/modify_stock.php?page_id=<?=$page_id; ?>&cat=<?=$category; ?>';" style="width: 100px; margin-top: 5px;" /></td>
