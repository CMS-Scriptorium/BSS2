...
if ($module_version < 0.70) {
    ...
    $itemstable = $database->query("SELECT * FROM {BXT}_items");
    ...
    if (!array_key_exists('option_attributes', $items)) {
        if ($database->query("ALTER TABLE `{BXT}_items` ADD `option_attributes` TEXT NOT NULL AFTER `shipping`")) {
        ...
        echo '<span class="ok">Database field option_attributes exists, update not needed</span><br />';
    }

    if (!array_key_exists('option_name', $items)) {
        if ($database->query("ALTER TABLE `{BXT}_items` ADD `option_name` VARCHAR(255) NOT NULL DEFAULT '' AFTER `shipping`")) {
            echo '<span class="good">Database field option_name added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field option_name exists, update not needed</span><br />';
    }

    // Get CUSTOMER table to see what needs to be created
    $customertable = $database->query("SELECT * FROM `{BXT}_customer`");
    $num_customers = $customertable->numRows();
    if ($num_customers == 0) {
        $database->query("INSERT INTO `{BXT}_customer` (order_id) VALUES ('0')");
    }
    $customertable = $database->query("SELECT * FROM `{BXT}_customer`");
    $customer = $customertable->fetchRow();

    // Modifying fields or adding new fields to the existing CUSTOMER table
    echo'<b>Modifying fields and adding new fields to the _customer table</b><br />';

    if (array_key_exists('cust_name', $customer)) {
        if ($database->query("ALTER TABLE `{BXT}_customer` CHANGE `cust_name` `cust_first_name` VARCHAR(50)")) {
            echo '<span class="good">Changed database field cust_name to cust_first_name successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field cust_last_name exists, update not needed</span><br />';
    }

    if (!array_key_exists('cust_last_name', $customer)) {
        if ($database->query("ALTER TABLE `{BXT}_customer` ADD `cust_last_name` VARCHAR(50) AFTER `cust_first_name`")) {
    ...
    $ordertable = $database->query("SELECT * FROM `{BXT}_order`");
    $num_orders = $ordertable->numRows();
    if ($num_orders == 0) {
        $database->query("INSERT INTO `{BXT}_order` (order_id) VALUES ('0')");
    }
    $ordertable = $database->query("SELECT * FROM `{BXT}_order`");
    ...
    if (!array_key_exists('attribute', $order)) {
        if ($database->query("ALTER TABLE `{BXT}_order` ADD `attribute` VARCHAR(50) NOT NULL AFTER `item_id`")) {
            echo '<span class="good">Database field attribute added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field attribute exists, update not needed</span><br />';
    }


    // Get SETTINGS table to see what needs to be created
    $settingstable = $database->query("SELECT * FROM `{BXT}_settings`");
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('offline_text', $settings)) {
            if ($database->query("ALTER TABLE `{BXT}_settings` ADD `offline_text` TINYTEXT NOT NULL AFTER `page_id`")) {
                echo '<span class="good">Database field offline_text added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field offline_text exists, update not needed</span><br />';
        }

        if (!array_key_exists('page_offline', $settings)) {
            if ($database->query("ALTER TABLE `{BXT}_settings` ADD `page_offline` ENUM('yes','no') NOT NULL DEFAULT 'no' AFTER `page_id`")) {
                echo '<span class="good">Database field page_offline added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field page_offline exists, update not needed</span><br />';
        }

        if (array_key_exists('shop_url', $settings)) {
            if ($database->query("ALTER TABLE `{BXT}_settings` CHANGE `shop_url` `proceed_url` VARCHAR(255) NOT NULL")) {
                echo '<span class="good">Changed database field shop_url to proceed_url successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field proceed_url exists, update not needed</span><br />';
        }

        if (array_key_exists('paypal_return', $settings)) {
            if ($database->query("ALTER TABLE `{BXT}_settings` DROP `paypal_return`")) {
                echo '<span class="good">Database field paypal_return deleted successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field paypal_return does not exist, update not needed</span><br />';
        }


        if ($database->query("ALTER TABLE `{BXT}_settings` CHANGE `shipping_method` `shipping_method` VARCHAR(20) NOT NULL")) {
        ...
        if (!array_key_exists('free_shipping_msg', $settings)) {
            if ($database->query("ALTER TABLE `{BXT}_settings` ADD `free_shipping_msg` ENUM('show','hide') NOT NULL DEFAULT 'hide' AFTER `shipping_method`")) {
                echo '<span class="good">Database field free_shipping_msg added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field free_shipping_msg exists, update not needed</span><br />';
        }


        if (!array_key_exists('free_shipping', $settings)) {
            if ($database->query("ALTER TABLE `{BXT}_settings` ADD `free_shipping` DECIMAL(6,2) NOT NULL AFTER `shipping_method`")) {
                echo '<span class="good">Database field free_shipping added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field free_shipping exists, update not needed</span><br />';
        }
    } else {
    ...
    $sSQL = 'CREATE TABLE `{BXT}_settings` ( '
    ...
    if ($settingstable = $database->query("SELECT * FROM `{BXT}_settings` ORDER BY `section_id` DESC LIMIT 1")) {
    ...
    if (empty($settings['free_shipping_msg'])) {
        $settings['free_shipping_msg'] = "all";
    } else {
        $free_shipping_msg = $settings['free_shipping_msg'];
    }
    ...
    if (empty($settings['page_offline'])) {
        $page_offline = "no";
    } else {
        $page_offline = $settings['page_offline'];
    }
    if (empty($settings['offline_text'])) {
        ...
        $offline_text = $TXT_BAKERY['ERR_OFFLINE_TEXT'];
    } else {
        $offline_text = $settings['offline_text'];
    }

    // Adding default settings to the new fields
    $query_dates = $database->query("SELECT * FROM `{BXT}_settings` WHERE `section_id` != 0 and `page_id` != 0");
    while ($result = $query_dates->fetchRow()) {

        echo '<br /><b>Adding default settings to database for bakery section_id=' . $result['section_id'] . '</b><br />';
        $section_id = $result['section_id'];

        if ($database->query("UPDATE `{BXT}_settings` SET `page_offline` = '$page_offline' WHERE `section_id` = $section_id")) {
            echo '<span class="good">Database data page_offline added successfully</span><br />';
        }
        echo '<span class="bad">' . $database->get_error() . '</span><br />';


        if ($database->query("UPDATE `{BXT}_settings` SET `offline_text` = '$offline_text' WHERE `section_id` = $section_id")) {
    ...
    if ($database->query("RENAME TABLE `{BXT}_settings` TO `{BXT}_page_settings`")) {
...
if ($module_version < 0.80) {
    ...
    if (!array_key_exists('tax_rate', $items)) {
        if ($database->query("ALTER TABLE {BXT}_items ADD tax_rate DECIMAL(5,3) NOT NULL AFTER shipping")) {
            echo '<span class="good">Database field tax_rate added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field tax_rate exists, update not needed</span><br />';
    }
    ...
    if (!array_key_exists('user_id', $customer)) {
        if ($database->query("ALTER TABLE {BXT}_customer ADD user_id INT(6) NOT NULL AFTER submitted")) {
            echo '<span class="good">Database field user_id added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field user_id exists, update not needed</span><br />';
    }


    if (!array_key_exists('cust_state', $customer)) {
        if ($database->query("ALTER TABLE {BXT}_customer ADD cust_state VARCHAR(50) NOT NULL AFTER cust_city")) {
            echo '<span class="good">Database field cust_state added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field cust_state exists, update not needed</span><br />';
    }
    ...
    if (!array_key_exists('tax_rate', $order)) {
        if ($database->query("ALTER TABLE {BXT}_order ADD tax_rate DECIMAL(5,3) NOT NULL AFTER price")) {
            echo '<span class="good">Database field attribute added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    ...
    $settingstable = $database->query("SELECT * FROM {BXT}_general_settings");
    $settings = $settingstable->fetchRow();
    ...
    if (!array_key_exists('zip_location', $settings)) {
        if ($database->query("ALTER TABLE {BXT}_general_settings ADD zip_location ENUM('inside','end') NOT NULL DEFAULT 'inside' AFTER shop_country")) {
            echo '<span class="good">Database field zip_location added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field zip_location exists, update not needed</span><br />';
    }


    if (!array_key_exists('state_field', $settings)) {
        if ($database->query("ALTER TABLE {BXT}_general_settings ADD state_field ENUM('show','hide') NOT NULL DEFAULT 'hide' AFTER shop_country")) {
            echo '<span class="good">Database field state_field added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field state_field exists, update not needed</span><br />';
    }


    if (!array_key_exists('tax_rate2', $settings)) {
        if ($database->query("ALTER TABLE {BXT}_general_settings ADD tax_rate2 DECIMAL(5,3) NOT NULL AFTER tax_rate")) {
            echo '<span class="good">Database field tax_rate2 added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field tax_rate2 exists, update not needed</span><br />';
    }


    if (!array_key_exists('tax_rate1', $settings)) {
        if ($database->query("ALTER TABLE {BXT}_general_settings ADD tax_rate1 DECIMAL(5,3) NOT NULL AFTER tax_rate")) {
            echo '<span class="good">Database field tax_rate1 added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field tax_rate1 exists, update not needed</span><br />';
    }


    if (!array_key_exists('zone_countries', $settings)) {
        if ($database->query("ALTER TABLE {BXT}_general_settings ADD zone_countries TEXT NOT NULL AFTER shipping_abroad")) {
            echo '<span class="good">Database field zone_countries added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field zone_countries exists, update not needed</span><br />';
    }


    if (!array_key_exists('shipping_zone', $settings)) {
        if ($database->query("ALTER TABLE {BXT}_general_settings ADD shipping_zone DECIMAL(6,2) NOT NULL AFTER shipping_abroad")) {
            echo '<span class="good">Database field shipping_zone added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field shipping_zone exists, update not needed</span><br />';
    }
    echo"<br />";
    ...
    // Get "old" settings
    if ($settingstable = $database->query("SELECT tax_rate FROM {BXT}_general_settings")) {
        $settings = $settingstable->fetchRow();
    ...
    // Insert values into general_settings table
    if ($database->query("UPDATE {BXT}_items SET tax_rate = '$tax_rate'")) {
...
if ($module_version < 0.81) {
    ...
    $settingstable = $database->query("SELECT * FROM {BXT}_page_settings");
    if ($settings = $settingstable->fetchRow()) {

        if (array_key_exists('proceed_url', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_page_settings CHANGE proceed_url continue_url VARCHAR(255) NOT NULL")) {
                echo '<span class="good">Changed database field proceed_url to continue_url successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field continue_url exists, update not needed</span><br />';
        }
    }
}
...
if ($module_version < 0.83) {
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('tax_rate_shipping', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD tax_rate_shipping DECIMAL(5,3) NOT NULL AFTER tax_included")) {
                echo '<span class="good">Database field tax_rate_shipping added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field tax_rate_shipping exists, update not needed</span><br />';
        }
    }
}
...
if ($module_version < 0.90) {
    ...
    $sSQL = 'CREATE TABLE {BXT}_options ( '
    ...
    $sSQL = 'CREATE TABLE {BXT}_attributes ( '
    ...
    $sSQL = 'CREATE TABLE {BXT}_item_attributes ( '
    ...
    if ($num_items == 0) {
        $database->query("INSERT INTO {BXT}_items (item_id) VALUES ('0')");
    }
    ...
    if ($item = $itemstable->fetchRow()) {
        ...
        if (!array_key_exists('stock', $item)) {
            if ($database->query("ALTER TABLE {BXT}_items ADD stock VARCHAR(20) NOT NULL DEFAULT '' AFTER sku")) {
                echo '<span class="good">Database field stock added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field stock exists, update not needed</span><br />';
        }
    }
    ...
    if ($customer = $customertable->fetchRow()) {
        ...
        if (array_key_exists('submitted', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer CHANGE submitted submitted VARCHAR(3) NOT NULL")) {
        ...
        if (!array_key_exists('status', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD status VARCHAR(20) NOT NULL DEFAULT 'none' AFTER submitted")) {
                echo '<span class="good">Database field status added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field status exists, update not needed</span><br />';
        }

        if (!array_key_exists('invoice', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD invoice TEXT NOT NULL AFTER cust_phone")) {
                echo '<span class="good">Database field invoice added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field invoice exists, update not needed</span><br />';
        }

        if (!array_key_exists('ship_zip', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD ship_zip VARCHAR(10) NOT NULL AFTER cust_phone")) {
                echo '<span class="good">Database field ship_zip added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field ship_zip exists, update not needed</span><br />';
        }

        if (!array_key_exists('ship_country', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD ship_country VARCHAR(2) NOT NULL AFTER cust_phone")) {
                echo '<span class="good">Database field ship_country added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field ship_country exists, update not needed</span><br />';
        }

        if (!array_key_exists('ship_state', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD ship_state VARCHAR(50) NOT NULL AFTER cust_phone")) {
                echo '<span class="good">Database field ship_state added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field ship_state exists, update not needed</span><br />';
        }

        if (!array_key_exists('ship_city', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD ship_city VARCHAR(50) NOT NULL AFTER cust_phone")) {
                echo '<span class="good">Database field ship_city added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field ship_city exists, update not needed</span><br />';
        }

        if (!array_key_exists('ship_street', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD ship_street VARCHAR(50) NOT NULL AFTER cust_phone")) {
                echo '<span class="good">Database field ship_street added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field ship_street exists, update not needed</span><br />';
        }

        if (!array_key_exists('ship_last_name', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD ship_last_name VARCHAR(50) NOT NULL AFTER cust_phone")) {
                echo '<span class="good">Database field ship_last_name added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field ship_last_name exists, update not needed</span><br />';
        }

        if (!array_key_exists('ship_first_name', $customer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD ship_first_name VARCHAR(50) NOT NULL AFTER cust_phone")) {
                echo '<span class="good">Database field ship_first_name added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field ship_first_name exists, update not needed</span><br />';
        }
    }
    ...
    if ($order = $ordertable->fetchRow()) {
        ...
        if (array_key_exists('attribute', $order)) {
            if ($database->query("ALTER TABLE {BXT}_order CHANGE attribute attributes VARCHAR(50) NOT NULL")) {
                echo '<span class="good">Changed database field attribute to attribute<b>s</b> successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }

            if ($database->query("UPDATE {BXT}_order SET attributes = ''")) {
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('shipping_form', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD shipping_form VARCHAR(10) NOT NULL AFTER shop_country")) {
                echo '<span class="good">Database field shipping_form added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field shipping_form exists, update not needed</span><br />';
        }

        if (!array_key_exists('invoice', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD invoice TEXT NOT NULL AFTER email_paypal")) {
                echo '<span class="good">Database field invoice added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        ...
        if (!array_key_exists('email_invoice', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD email_invoice TEXT NOT NULL AFTER email_paypal")) {
                echo '<span class="good">Database field email_invoice added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field email_invoice exists, update not needed</span><br />';
        }

        if (!array_key_exists('email_subject_invoice', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD email_subject_invoice TEXT NOT NULL AFTER email_paypal")) {
                echo '<span class="good">Database field email_subject_invoice added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field email_subject_invoice exists, update not needed</span><br />';
        }
    }
    ...
    $email_subject_invoice = $TXT_BAKERY[$payment_method]['EMAIL_SUBJECT_CUSTOMER'];
    $email_invoice = $TXT_BAKERY[$payment_method]['EMAIL_BODY_CUSTOMER'];
    $invoice = $database->escapeString($TXT_BAKERY[$payment_method]['INVOICE_TEMPLATE']);

    if ($database->query("UPDATE {BXT}_general_settings SET email_subject_invoice = '$email_subject_invoice', email_invoice = '$email_invoice', invoice = '$invoice'")) {
...
if ($module_version < 0.96) {
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('shop_state', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD shop_state VARCHAR(2) NOT NULL AFTER shop_country")) {
                echo '<span class="good">Database field shop_state added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field shop_state exists, update not needed</span><br />';
        }

        if (!array_key_exists('tax_by', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD tax_by VARCHAR(10) NOT NULL AFTER tax_included")) {
                echo '<span class="good">Database field tax_by added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field tax_by exists, update not needed</span><br />';
        }

        // Insert default settings into general settings table
        if ($database->query("UPDATE {BXT}_general_settings SET tax_by = 'country'")) {
            echo '<span class="good">Added default general settings successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
...
if ($module_version < 0.97) {
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('definable_field_0', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD definable_field_0 VARCHAR(50) NOT NULL AFTER use_captcha")) {
                echo '<span class="good">Database field definable_field_0 added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field definable_field_0 exists, update not needed</span><br />';
        }

        if (!array_key_exists('definable_field_1', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD definable_field_1 VARCHAR(50) NOT NULL AFTER definable_field_0")) {
                echo '<span class="good">Database field definable_field_1 added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field definable_field_1 exists, update not needed</span><br />';
        }

        if (!array_key_exists('definable_field_2', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD definable_field_2 VARCHAR(50) NOT NULL AFTER definable_field_1")) {
                echo '<span class="good">Database field definable_field_2 added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field definable_field_2 exists, update not needed</span><br />';
        }

        if (!array_key_exists('stock_mode', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD stock_mode VARCHAR(10) NOT NULL AFTER definable_field_2")) {
                echo '<span class="good">Database field stock_mode added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field stock_mode exists, update not needed</span><br />';
        }

        if (!array_key_exists('stock_limit', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD stock_limit INT(3) NOT NULL AFTER stock_mode")) {
                echo '<span class="good">Database field stock_limit added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field stock_limit exists, update not needed</span><br />';
        }
    }

    // Change continue_url to a link not containing domain name nor page directory
    echo "<br /><b>Modifying table page_settings:</b><br />";
    $settingstable = $database->query("SELECT section_id, continue_url FROM {BXT}_page_settings");
    while ($settings = $settingstable->fetchRow()) {
        ...
        if ($database->query("UPDATE {BXT}_page_settings SET continue_url = '$continue_url' WHERE section_id = '$section_id'")) {
    ...
    if ($item = $itemstable->fetchRow()) {
        ...
        if (!array_key_exists('definable_field_0', $item)) {
            if ($database->query("ALTER TABLE {BXT}_items ADD definable_field_0 VARCHAR(150) NOT NULL AFTER tax_rate")) {
                echo '<span class="good">Database field definable_field_0 added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        ...
        if (!array_key_exists('definable_field_1', $item)) {
            if ($database->query("ALTER TABLE {BXT}_items ADD definable_field_1 VARCHAR(150) NOT NULL AFTER definable_field_0")) {
                echo '<span class="good">Database field definable_field_1 added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        ...
        if (!array_key_exists('definable_field_2', $item)) {
            if ($database->query("ALTER TABLE {BXT}_items ADD definable_field_2 VARCHAR(150) NOT NULL AFTER definable_field_1")) {
                echo '<span class="good">Database field definable_field_2 added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
    ...
    while ($item = $itemstable->fetchRow()) {
        ...
        if ($database->query("UPDATE {BXT}_items SET link = '$link' WHERE item_id = '$item_id'")) {
...
if ($module_version < 1.10) {
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('display_settings', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD display_settings ENUM('1','0') NOT NULL DEFAULT '1' AFTER zip_location")) {
                echo '<span class="good">Database field display_settings added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field display_settings exists, update not needed</span><br />';
        }

        if (!array_key_exists('out_of_stock_orders', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD out_of_stock_orders ENUM('1','0') NOT NULL DEFAULT '0' AFTER stock_limit")) {
                echo '<span class="good">Database field out_of_stock_orders added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field out_of_stock_orders exists, update not needed</span><br />';
        }


        if (!array_key_exists('thousands_sep', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD thousands_sep VARCHAR(1) NOT NULL DEFAULT '\'' AFTER shop_currency")) {
                echo '<span class="good">Database field thousands_sep added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field thousands_sep exists, update not needed</span><br />';
        }

        if (!array_key_exists('dec_point', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD dec_point VARCHAR(1) NOT NULL DEFAULT '.' AFTER shop_currency")) {
                echo '<span class="good">Database field dec_point added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field dec_point exists, update not needed</span><br />';
        }


        if (!array_key_exists('skip_checkout', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD skip_checkout ENUM('1','0') NOT NULL DEFAULT '0' AFTER tax_included")) {
                echo '<span class="good">Database field skip_checkout added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field skip_checkout exists, update not needed</span><br />';
        }
    }
    ...
    $sSQL = 'CREATE TABLE {BXT}_payment_methods ( '
    ...
    if (!file_exists(WB_PATH . '/modules/bakery/payment_methods/load.php')) {
        ...
        if ($settingstable = $database->query("SELECT * FROM {BXT}_general_settings")) {
            $settings = $settingstable->fetchRow();
            extract($settings);
            ...
            foreach ($load_payment_methods as $payment_method) {
                switch ($payment_method) {
                    case 'advance':
                        if ($database->query("UPDATE {BXT}_payment_methods SET cust_email_subject = '$email_subject_advance', cust_email_body = '$email_pay_advance' WHERE directory = 'advance'")) {
                        ...
                        break;
                    case 'invoice':
                        if ($database->query("UPDATE {BXT}_payment_methods SET value_1 = '$bank_account', value_4 = '$invoice', cust_email_subject = '$email_subject_invoice', cust_email_body = '$email_invoice' WHERE directory = 'invoice'")) {
                        ...
                        break;
                    case 'paypal':
                        if ($database->query("UPDATE {BXT}_payment_methods SET value_1 = '$paypal_email', value_2 = '$paypal_page', cust_email_subject = '$email_subject_paypal', cust_email_body = '$email_paypal' WHERE directory = 'paypal'")) {
    ...
    if ($moved_successfully) {
        if ($database->query("ALTER TABLE {BXT}_general_settings DROP payment_method, DROP bank_account, DROP paypal_email, DROP paypal_page, DROP email_subject_advance, DROP email_pay_advance, DROP email_subject_paypal, DROP email_paypal, DROP email_subject_invoice, DROP email_invoice, DROP invoice")) {
    ...
    if ($database->query("ALTER TABLE {BXT}_customer CHANGE submitted submitted VARCHAR(20) NOT NULL DEFAULT 'no'")) {
    ...
    if (!array_key_exists('transaction_status', $customer)) {
        if ($database->query("ALTER TABLE {BXT}_customer ADD transaction_status VARCHAR(10) NOT NULL DEFAULT 'none' AFTER submitted")) {
            echo '<span class="good">Database field transaction_status added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field transaction_status exists, update not needed</span><br />';
    }

    if (!array_key_exists('transaction_id', $customer)) {
        if ($database->query("ALTER TABLE {BXT}_customer ADD transaction_id VARCHAR(50) NOT NULL DEFAULT 'none' AFTER submitted")) {
            echo '<span class="good">Database field transaction_id added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field transaction_id exists, update not needed</span><br />';
    }
    ...
    foreach ($payment_methods as $abbr => $payment_method) {
        if ($database->query("UPDATE {BXT}_customer SET submitted = '$payment_method' WHERE submitted = '$abbr'")) {
...
if ($module_version < 1.30) {
    ...
    if (!array_key_exists('main_image', $items)) {
        if ($database->query("ALTER TABLE {BXT}_items CHANGE extension main_image VARCHAR(50) NOT NULL")) {
            echo '<span class="good">Changed database field extension to main_image successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field main_image exists, update not needed</span><br />';
    }
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('lightbox2', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_page_settings ADD lightbox2 VARCHAR(10) NOT NULL DEFAULT 'detail' AFTER resize")) {
                echo '<span class="good">Database field lightbox2 added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field lightbox2 exists, update not needed</span><br />';
        }

        if ($database->query("UPDATE {BXT}_page_settings SET resize = '100' WHERE resize = 0")) {
...
if ($module_version < 1.40) {
    ...
    if ($database->query("UPDATE {BXT}_payment_methods SET value_4 = value_2, field_4 = field_2, value_2 = '', field_2 = '' WHERE directory = 'invoice' AND version = '0.1' LIMIT 1")) {
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('skip_cart', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD skip_cart ENUM('yes',  'no') NOT NULL DEFAULT 'no' AFTER zip_location")) {
                echo '<span class="good">Database field skip_cart added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field skip_cart exists, update not needed</span><br />';
        }
    }


    // Convert continue_url from string link to numeric page_id
    if ($database->query("UPDATE {BXT}_page_settings ps SET ps.continue_url = (SELECT p.page_id FROM {TP}pages p WHERE ps.continue_url = p.link) WHERE LEFT(continue_url, 1) = '/'")) {
    ...
    $replace_counter = 0;
    $query_page_settings = $database->query("SELECT section_id, header, item_loop, footer, item_header, item_footer FROM {BXT}_page_settings");
    if ($query_page_settings->numRows() > 0) {
        while ($page_settings = $query_page_settings->fetchRow()) {
            $page_settings = array_map('lazystrip', $page_settings);
            ...
            if ($database->query("UPDATE {BXT}_page_settings SET $update_string WHERE section_id = '{$page_settings['section_id']}'")) {
...
if ($module_version < 1.51) {
    ...
    if (!array_key_exists('pages_directory', $settings)) {
        if ($database->query("ALTER TABLE {BXT}_general_settings ADD pages_directory VARCHAR(20) NOT NULL DEFAULT 'bakery' AFTER shop_email")) {
            echo '<span class="good">Database field pages_directory added successfully</span><br />';
        } else {
            echo '<span class="bad">' . $database->get_error() . '</span><br />';
        }
    } else {
        echo '<span class="ok">Database field pages_directory exists, update not needed</span><br />';
    }
}
...
if ($module_version < 1.60) {
    ...
    if ($costumer = $customertable->fetchRow()) {
        ...
        if (!array_key_exists('cust_tax_no', $costumer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD cust_tax_no VARCHAR(11) NOT NULL AFTER cust_last_name;")) {
                echo '<span class="good">Database field cust_tax_no added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field cust_tax_no exists, update not needed</span><br />';
        }
    }
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('tax_no_field', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD tax_no_field ENUM('show', 'hide') NOT NULL AFTER state_field")) {
                echo '<span class="good">Database field tax_no_field added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field tax_no_field exists, update not needed</span><br />';
        }
        ...
        if (!array_key_exists('tax_group', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD tax_group VARCHAR(255) NOT NULL DEFAULT '$cfg_tax_group' AFTER tax_no_field")) {
                echo '<span class="good">Database field tax_group added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field tax_group exists, update not needed</span><br />';
        }
    }
}
...
if ($module_version < 1.70) {
    ...
    if ($costumer = $customertable->fetchRow()) {
        ...
        if (!array_key_exists('cust_company', $costumer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD cust_company VARCHAR(50) NOT NULL AFTER user_id;")) {
                echo '<span class="good">Database field cust_company added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field cust_company exists, update not needed</span><br />';
        }

        if (!array_key_exists('ship_company', $costumer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD ship_company VARCHAR(50) NOT NULL AFTER cust_phone;")) {
                echo '<span class="good">Database field ship_company added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field ship_company exists, update not needed</span><br />';
        }

        if (!array_key_exists('invoice_id', $costumer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD invoice_id INT(6) NOT NULL AFTER ship_zip;")) {
                echo '<span class="good">Database field invoice_id added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field invoice_id exists, update not needed</span><br />';
        }
    }
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('company_field', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD company_field ENUM('show','hide') NOT NULL AFTER shipping_form")) {
                echo '<span class="good">Database field company_field added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field company_field exists, update not needed</span><br />';
        }

        if (!array_key_exists('cust_msg', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD cust_msg ENUM('show','hide') NOT NULL AFTER zip_location")) {
                echo '<span class="good">Database field cust_msg added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field cust_msg exists, update not needed</span><br />';
        }

        if ($database->query("ALTER TABLE {BXT}_general_settings CHANGE shop_state shop_state VARCHAR(5) NOT NULL")) {
        ...
        if (array_key_exists('skip_checkout', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings DROP skip_checkout")) {
                echo '<span class="good">Database field skip_checkout deleted successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field skip_checkout does not exist, update not needed</span><br />';
        }
    }
    ...
    if ($items = $itemstable->fetchRow()) {
        ...
        if (!array_key_exists('created_when', $items)) {
            if ($database->query("ALTER TABLE {BXT}_items ADD created_when INT NOT NULL DEFAULT '0' AFTER modified_by")) {
                echo '<span class="good">Database field created_when added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field created_when exists, update not needed</span><br />';
        }

        if (!array_key_exists('created_by', $items)) {
            if ($database->query("ALTER TABLE {BXT}_items ADD created_by INT NOT NULL DEFAULT '0' AFTER created_when")) {
                echo '<span class="good">Database field created_by added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field created_by exists, update not needed</span><br />';
        }

        if (array_key_exists('main_image', $items)) {
            if ($database->query("ALTER TABLE {BXT}_items DROP main_image")) {
                echo '<span class="good">Database field main_image deleted successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field main_image does not exist, update not needed</span><br />';
        }
    }
...
if ($module_version < 1.72) {
    ...
    if ($costumer = $customertable->fetchRow()) {
        ...
        if (!array_key_exists('sent_invoices', $costumer)) {
            if ($database->query("ALTER TABLE {BXT}_customer ADD sent_invoices INT(1) NOT NULL DEFAULT '0' AFTER invoice_id;")) {
                echo '<span class="good">Database field sent_invoices added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field sent_invoices exists, update not needed</span><br />';
        }
    }
}
...
if ($module_version < 1.79) {
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('no_revocation', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD no_revocation VARCHAR(50) NOT NULL DEFAULT 'e-goods' AFTER zip_location")) {
                echo '<span class="good">Database field no_revocation added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field no_revocation exists, update not needed</span><br />';
        }

        if (!array_key_exists('hide_country', $settings)) {
            if ($database->query("ALTER TABLE {BXT}_general_settings ADD hide_country ENUM('show','hide') NOT NULL DEFAULT 'show' AFTER no_revocation")) {
                echo '<span class="good">Database field hide_country added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field hide_country exists, update not needed</span><br />';
        }
    }
}
...
if ($module_version < 1.80) {
    ...
    if ($database->query($sql)) {
        echo '<span class="good">Added default values to the {BXT}_customer table successfully</span><br />';
    } else {
        echo '<span class="bad">' . $database->get_error() . '</span><br />';
    }

    // Add default values to the GENERAL SETTINGS table to make it compatible with mysql save mode
    $sql = "ALTER TABLE `{BXT}_general_settings`
    ...
    if ($database->query($sql)) {
        echo '<span class="good">Added default values to the {TP}mod_general_settings table successfully</span><br />';
    } else {
        echo '<span class="bad">' . $database->get_error() . '</span><br />';
    }

    // Add default values to the PAGE SETTINGS table to make it compatible with mysql save mode
    $sql = "ALTER TABLE `{BXT}_page_settings`
    ...
    if ($database->query($sql)) {
        echo '<span class="good">Added default values to the {BXT}_page_settings table successfully</span><br />';
    } else {
        echo '<span class="bad">' . $database->get_error() . '</span><br />';
    }

    // Add default values to the PAYMENT METHODS table to make it compatible with mysql save mode
    $sql = "ALTER TABLE `{BXT}_payment_methods`
    ...
    if ($database->query($sql)) {
        echo '<span class="good">Added default values to the {BXT}_payment_methods table successfully</span><br />';
    } else {
        echo '<span class="bad">' . $database->get_error() . '</span><br />';
    }

    // Add default values to the ITEMS table to make it compatible with mysql save mode
    $sql = "ALTER TABLE `{BXT}_items`
    ...
    if ($database->query($sql)) {
        echo '<span class="good">Added default values to the {BXT}_items table successfully</span><br />';
    } else {
        echo '<span class="bad">' . $database->get_error() . '</span><br />';
    }

    // Add default values to the IMAGES table to make it compatible with mysql save mode
    $sql = "ALTER TABLE `{BXT}_images`
                CHANGE `alt` `alt` VARCHAR(255) NULL DEFAULT NULL,
                CHANGE `title` `title` VARCHAR(255) NULL DEFAULT NULL,
                CHANGE `caption` `caption` TEXT NULL DEFAULT NULL;";
    if ($database->query($sql)) {
        echo '<span class="good">Added default values to the {BXT}_images table successfully</span><br />';
    } else {
        echo '<span class="bad">' . $database->get_error() . '</span><br />';
    }
}
...
if ($module_version < 1.85) {
    // Titel: Upgrading to
    echo'<h3>Upgrading to version 1.85 or later:</h3>';
    ...
    if ($settings = $settingstable->fetchRow()) {
        ...
        if (!array_key_exists('cancellation_url', $settings)) {
            if ($database->query("ALTER TABLE `{BXT}_general_settings` ADD `cancellation_url` VARCHAR(255) NULL DEFAULT NULL AFTER `no_revocation`")) {
                echo '<span class="good">Database field cancellation_url added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field cancellation_url exists, update not needed</span><br />';
        }

        if (!array_key_exists('privacy_url', $settings)) {
            if ($database->query("ALTER TABLE `{BXT}_general_settings` ADD `privacy_url` VARCHAR(255) NULL DEFAULT NULL AFTER `cancellation_url`")) {
                echo '<span class="good">Database field privacy_url added successfully</span><br />';
            } else {
                echo '<span class="bad">' . $database->get_error() . '</span><br />';
            }
        } else {
            echo '<span class="ok">Database field privacy_url exists, update not needed</span><br />';
        }
    }
} 