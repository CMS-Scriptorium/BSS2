...
foreach ($avaiable_payment_methods as $payment_method_dir) {
	$query_payment_methods = $database->query("SELECT version FROM {BXT}_payment_methods WHERE directory = '$payment_method_dir'");
...
if (!empty($load_payment_methods)) {
	foreach ($load_payment_methods as $payment_method_dir) {
		...
		$query_payment_methods = $database->query("SELECT * FROM {BXT}_payment_methods WHERE directory = '$payment_method_dir'");
		$payment_method = $query_payment_methods->fetchRow();
		
		if ($query_payment_methods->numRows() > 0) {

			// Upgrade payment method (mysql update)
			$database->query("UPDATE {BXT}_payment_methods SET name = '$payment_method_name', version = '$payment_method_version', author = '$payment_method_author', requires = '$requires_bakery_module', field_1 = '$field_1', field_2 = '$field_2', field_3 = '$field_3', field_4 = '$field_4', field_5 = '$field_5', field_6 = '$field_6' WHERE directory = '$payment_method_dir'");
			...
			// Prepare email vars
			$cust_email_subject = $TXT_BAKERY[$payment_method_dir]['EMAIL_SUBJECT_CUSTOMER'];
			$cust_email_body = $TXT_BAKERY[$payment_method_dir]['EMAIL_BODY_CUSTOMER'];
			$shop_email_subject = $TXT_BAKERY[$payment_method_dir]['EMAIL_SUBJECT_SHOP'];
			$shop_email_body = $TXT_BAKERY[$payment_method_dir]['EMAIL_BODY_SHOP'];

			// Insert with invoice template (value_4)
			if ($payment_method_dir == "invoice") {
				$value_4 = $TXT_BAKERY[$payment_method_dir]['INVOICE_TEMPLATE'];
				$database->query("INSERT INTO {BXT}_payment_methods (active, directory, name, version, author, requires, field_1, field_2, field_3, field_4, value_4, field_5, field_6, cust_email_subject, cust_email_body, shop_email_subject, shop_email_body) VALUES ('0', '$payment_method_dir', '$payment_method_name', '$payment_method_version', '$payment_method_author', '$requires_bakery_module', '$field_1', '$field_2', '$field_3', '$field_4', '$value_4', '$field_5', '$field_6', '$cust_email_subject', '$cust_email_body', '$shop_email_subject', '$shop_email_body')");
			} else {
