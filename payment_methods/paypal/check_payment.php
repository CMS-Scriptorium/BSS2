...
$order_total = $_SESSION['bxt']['order_total'];

// Get PayPal email (business var) and authentication token from db
$query_payment_methods = $database->query("SELECT value_1, value_3 FROM {BXT}_payment_methods WHERE directory = 'paypal'");
...
$transaction_id = $database->get_one("SELECT transaction_id FROM {BXT}_customer WHERE order_id = '$order_id'");
...
if (!$res) {
	...
	if (strcmp($lines[0], 'SUCCESS') == 0) {
		...
		if (count($errors) == 0) {
			...
			$database->query("UPDATE {BXT}_customer SET transaction_status = 'paid', transaction_id = '$tx_token' WHERE order_id = '$order_id' AND transaction_id = 'none' AND transaction_status IN ('none','pending')");
		...
		// ERROR
			...
			$database->query("UPDATE {BXT}_customer SET transaction_status = 'pending', transaction_id = '$tx_token' WHERE order_id = '$order_id' AND transaction_id = 'none' AND transaction_status = 'none'");
	...
	elseif (strcmp($lines[0], "FAIL") == 0) {
		...
		// Set payment status pending and update db
		$database->query("UPDATE {BXT}_customer SET transaction_status = 'pending' WHERE order_id = '$order_id' AND transaction_id = 'none' AND transaction_status = 'none'");
		$payment_status = "pending";
		return;
	}
}
