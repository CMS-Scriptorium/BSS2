...
$query_payment_methods = $database->query("SELECT value_1, value_2, value_4 FROM {BXT}_payment_methods WHERE directory = 'payment-network'");
...
if (isset($_POST)) {
	if ($_POST['user_id'] == $user_id && $_POST['project_id'] == $project_id) {
		...
		if ($_POST['hash'] == $hash) {
			...
			if ($database->query("UPDATE {BXT}_customer SET transaction_status = 'paid', transaction_id = '$transaction_id' WHERE order_id = '$order_id'")) {
