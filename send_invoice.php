...
$display_invoice       = '';
$title                 = $TXT_BAKERY['INVOICE'];
$charset               = defined('DEFAULT_CHARSET') ? DEFAULT_CHARSET : 'utf-8';


// Get invoice template from db payment methods table
$query_payment_methods = $database->query("SELECT `value_4` FROM `{BXT}_payment_methods` WHERE `directory` = 'invoice'");
if ($query_payment_methods->numRows() > 0) {
	$payment_methods = $query_payment_methods->fetchRow();
}
$invoice_template = lazystrip($payment_methods['value_4']);

// Get invoice data string from db customer table
$query_customer = $database->query("SELECT `invoice_id`, `sent_invoices`, `invoice`, `submitted`, `transaction_status` FROM `{BXT}_customer` WHERE `order_id` = '$order_id'");
if ($query_customer->numRows() > 0) {
	...
	if (!empty($customer['invoice'])) {
		...
		$sent_invoices = lazystrip($customer['sent_invoices']);		
		...
		$invoice_address      = lazyexplode('<br /><br />', $invoice_array[4]);
		$invoice_address      = $invoice_address[0];
		$invoice_cust_address = lazyexplode('<br /><br />', $invoice_array[5]);
		...
		if (LANGUAGE_LOADED) {
		    include_once __DIR__.'/payment_methods/'.$payment_method.'/languages/EN.php';
		    if (file_exists(__DIR__.'/payment_methods/'.$payment_method.'/languages/'.LANGUAGE.'.php')) {
		        include_once __DIR__.'/payment_methods/'.$payment_method.'/languages/'.LANGUAGE.'.php';
		    }
		}

		// Email subject
		$email_subject = $shop_name.' '.$TXT_BAKERY['INVOICE'];
		...
		$values = array(WB_URL, $order_id, $invoice_id, $shop_name, $bank_account, $cust_name, $invoice_address, $invoice_cust_address, $invoice_ship_address, $cust_email, $html_item_list, $order_date, $current_date, $title, $display_invoice, $display_delivery_note, $display_reminder, $cust_tax_no, $TXT_BAKERY[$payment_method]['NAME']);
...
';
		...
		$mailer->setFrom($shop_email,$shop_name);
		$mailer->Subject = $email_subject;
		$mailer->Body = $html;
		if($mailer->send()){
			// On success increment email counter
			$database->query("UPDATE `{BXT}_customer` SET `sent_invoices` = `sent_invoices` + '1' WHERE order_id = '$order_id'");
			// On success view confirmation
			$admin->print_success($TXT_BAKERY['INVOICE_HAS_BEEN_SENT_SUCCESSFULLY'], WB_URL.'/modules/bakery/modify_orders.php?page_id='.$page_id);
		} else {
