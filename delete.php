...
$query_items = $database->query("SELECT item_id, link FROM {BXT}_items WHERE section_id = '$section_id'");
if ($query_items->numRows() > 0) {
	while ($item = $query_items->fetchRow()) {
		...
		$database->query("DELETE FROM {BXT}_item_attributes WHERE item_id = '{$item['item_id']}'");
	}
}

// Delete items and page settings in db
$database->query("DELETE FROM {BXT}_items WHERE section_id = '$section_id'");
$database->query("DELETE FROM {BXT}_page_settings WHERE section_id = '$section_id'");
