...
if (isset($_SESSION['bxt']['order_id']) && is_numeric($_SESSION['bxt']['order_id']) && $_SESSION['bxt']['order_id'] >= 0) {
	...
	if ($num_orders > 0) {
		...
		// Get continue url
		$query_continue_url = $database->query("SELECT p.link FROM {TP}pages p INNER JOIN {BXT}_page_settings ps ON p.page_id = ps.page_id $clause");
		if ($query_continue_url->numRows() > 0) {
			$fetch_continue_url = $query_continue_url->fetchRow();
			$continue_url = WB_URL.PAGES_DIRECTORY.lazystrip($fetch_continue_url['link']).PAGE_EXTENSION;
		}

		// Get the general settings
		$query_general_settings = $database->query("SELECT shop_currency, dec_point, thousands_sep, tax_included FROM {BXT}_general_settings");
		if ($query_general_settings->numRows() > 0) {
			$general_settings = $query_general_settings->fetchRow();
			$shop_currency = lazystrip($general_settings['shop_currency']);
			$dec_point = lazystrip($general_settings['dec_point']);
			$thousands_sep = lazystrip($general_settings['thousands_sep']);
			$tax_included = lazystrip($general_settings['tax_included']);
		}
		...
		if ($tax_included == 'included') {
			$txt_excl_shipping_tax  = $TXT_BAKERY['EXCL_SHIPPING'];
		}
		else {
			// Calculate tax amount for shipping excluding tax (netto)
			$txt_excl_shipping_tax  = $TXT_BAKERY['EXCL_SHIPPING_TAX'];
		}
