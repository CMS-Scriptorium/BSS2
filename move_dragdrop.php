...
if (! isset($_POST['action']) || !isset($_POST['row']) ) { 
	...
	$admin = new admin('Modules', 'modules_view', false, false);
	...
	$row = $_POST['row'];	
	$sID = $database->get_one("SELECT `section_id` FROM `{BXT}_items` WHERE `item_id` = ".intval($row[0]));
	
	
	/*
	Bakery isn't using ordering (ASC/DESC) so we comment this code

	$sorting = $database->get_one("SELECT ordering FROM {TP}bakery_settings WHERE section_id = ".$sID." ");
	...
	*/
	...
	if ($action == "updatePosition") {
		...
		foreach ($row as $recID) {
			// Sanitize array
			$recID = intval($recID);
			$database->query("UPDATE `{BXT}_items` SET `position` = ".$i." WHERE `item_id` = ".$recID." ");
			$i++;
		}
	
		// Include ordering class and reorder the entries
		require_once(WB_PATH.'/framework/class.order.php');
		$order = new order('{BXT}_items', 'position', 'item_id', 'section_id');
		$order->clean($sID);
