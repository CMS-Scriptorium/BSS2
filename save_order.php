...
$invoice = $database->get_one("SELECT `invoice` FROM `{BXT}_customer` WHERE `order_id` = '$order_id'");

// Convert invoice data string to an array
if (!empty($invoice)) {
    $invoice = lazystrip($invoice);
...
foreach ($_POST as $field => $value) {
    $skip_fields = array('page_id', 'section_id', 'order_id', 'save_and_return', 'save', 'save_form');
...
$query_general_settings = $database->query("SELECT `shop_country`, `state_field`, `zip_location`, `hide_country` FROM `{BXT}_general_settings`");
if ($query_general_settings->numRows() > 0) {
    $general_settings     = $query_general_settings->fetchRow();
    $setting_shop_country = lazystrip($general_settings['shop_country']);
    $setting_state_field  = lazystrip($general_settings['state_field']);
    $setting_zip_location = lazystrip($general_settings['zip_location']);
    $setting_hide_country = lazystrip($general_settings['hide_country']);
}
...
// Update db
$database->query(
    "UPDATE `{BXT}_customer` SET $update_string WHERE `order_id` = '$order_id'"
);
